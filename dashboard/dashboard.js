...
function toListItem(text, subject, start, end, googleFiles, objekt_id, rooms, teachers) {
	...
	for (i = 0; i < googleFiles; i++) {
		if (i < entriesToAdd.length) {
			...
			var uddatalink = "https://all.uddataplus.dk/skema/?id=id_skema#u:e!" + objekt_id + "!" + toCompIsoString(startDate);
...
$(document).keydown(function(e) {
	if (!$("#searchBox").is(":focus")) {
		if (e.which == 37) {
			...
			//B
			window.open("https://all.uddataplus.dk/opgave");
		} else if (e.which == 67) {
			//C
			window.open("https://all.uddataplus.dk/besked");
		} else if (e.which == 82) {
			//R
			window.open("https://all.uddataplus.dk/ressourcer");
		}
